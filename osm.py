...
NO_RESULTS = (
    "No results found. Tell the user you found no results. "
    "Do not make up answers or hallucinate. Only say you "
    "found no results."
)

NO_RESULTS_BAD_ADDRESS = (
    "No results found. Tell the user you found no results because "
...
NO_CONFUSION = (
    "**IMPORTANT!:** Check that the results match the location "
...
def convert_and_validate_results(
    ...
    for thing in things_nearby:
        ...
        distance = (
            f" - Haversine Distance from Origin: {friendly_thing['distance']} km\n"
            if use_distance
            else ""
        )
        travel_distance = (
            f" - Travel Distance from Origin: {friendly_thing['nav_distance']}\n"
            if use_distance and "nav_distance" in friendly_thing
            else ""
        )
        map_link = create_osm_link(friendly_thing["lat"], friendly_thing["lon"])
        entry = (
            f"## {friendly_thing['name']}\n"
...
class OrsRouter:
    def __init__(
        ...
        if "ors_api_key" in self.valves and self.valves.ors_api_key != "":
            if "ors_instance" in self.valves:
                self._client = openrouteservice.Client(
                    base_url=self.valves.ors_instance, key=self.valves.ors_api_key
                )
            else:
    ...
    def calculate_route(self, from_thing: dict, to_thing: dict) -> Optional[dict]:
        ...
        if not self.valves.car_only and to_thing.get("distance", 9000) <= 1.5:
...
class OsmSearcher:
    ...
    def use_detailed_interpretation_mode(self) -> bool:
        ...
        # setting.
        if self.user_valves is not None and "instruction_oriented_interpretation" in self.user_valves:
    ...
    async def search_nearby(
        ...
        try:
            ...
            if search_results:
                ...
                result_instructions = (
                    "No results found at all. " "Tell the user there are no results."
                )

            resp = f"{result_instructions}\n\n" f"{search_results}"
            ...
            result = (
                f"No results were found, because of an error. "
...
async def do_osm_search(
    ...
    search = await searcher.search_nearby(
        place, tags, limit=limit, radius=radius, category=category
    )
...
class OsmNavigator:
    ...
    async def navigate(self, start_place: str, destination_place: str):
        ...
        try:
            ...
            await self.event_error(e)
            return (
                f"There are no results due to an error. "
...
async def find_address_for_coordinates(
     latitude: Annotated[float, Field(description="The latitude portion of the GPS coordinate.")],
     longitude: Annotated[float, Field(description="The longitude portion of the GPS coordinate.")], ctx: Context)->Annotated[str, Field(description="Information about the address or place at the coordinates.")]:
    """
...
async def find_store_or_place_near_coordinates(
    store_or_business_name: Annotated[str, Field(description="Name of store or business to look for.")],
    latitude: Annotated[float, Field(description="The latitude portion of the GPS coordinate.")],
    longitude: Annotated[float, Field(description="The longitude portion of the GPS coordinate.")], ctx: Context) -> Annotated[str, Field(description="Information about the address or places near the coordinates.")]:
    """
...
async def find_specific_place(
     address_or_place: Annotated[str, Field(description="The address or place to look up.")], ctx: Context) -> Annotated[str, Field(description="Information about the place, if found.")]:
    """
...
async def navigate_between_places(
    start_address_or_place: Annotated[str, Field(description="The address, place, or coordinates to start from.")],
    destination_address_or_place: Annotated[str, Field(description="The destination address, place, or coordinates to go.")]) -> Annotated[str, Field(description="The navigation route and associated info, if found.")]:
    """
...
async def find_drugs_near_place(
    ...
    return await do_osm_search(
        valves=valves,
        user_valves=user_valves,
        category="cannabis and smartshops",
        place=place,
        tags=tags,
    )
...
async def find_electronics_store_near_place(
    ...
    return await do_osm_search(
        valves=valves,
        user_valves=user_valves,
        category="consumer electronics stores",
        place=place,
        tags=tags,
    )
...
def find_other_things_near_place(
    place: Annotated[str, Field(description="The name of a place, an address, or GPS coordinates. City and country must be specified, if known.")], category: Annotated[str, Field(description="The category of place, shop, etc to look up.")]
) -> Annotated[str, Field(description="A list of nearby shops.")]:
    """
